# Fix all remaining OfferService.cs issues
$filePath = "backend/InsuranceAPI/Services/OfferService.cs"
$content = Get-Content $filePath -Raw

# Fix the decimal ?? int issue in AgentId assignment
$content = $content -replace 'AgentId = createOfferDto\.AgentId \?\? 0', 'AgentId = createOfferDto.AgentId'

# Fix the decimal ?? decimal issue in DiscountRate
$content = $content -replace 'DiscountRate = createOfferDto\.DiscountRate \?\? 0', 'DiscountRate = createOfferDto.DiscountRate ?? 0'

# Fix the decimal ?? decimal issue in FinalPrice calculation
$content = $content -replace 'FinalPrice = insuranceType\.BasePrice \* \(1 - \(createOfferDto\.DiscountRate \?\? 0\) / 100\)', 'FinalPrice = insuranceType.BasePrice * (1 - (createOfferDto.DiscountRate ?? 0) / 100)'

# Replace the entire MapToDto method with the correct flat property mapping
$oldMapToDto = @'
...
$content = $content -replace [regex]::Escape($oldMapToDto), $newMapToDto

Set-Content $filePath $content
Write-Host "Fixed all remaining OfferService.cs issues"
