...
$content = $content -replace 'customer\.Id', 'customer.CustomerId'
...
# Fix the decimal ?? int issue
$content = $content -replace '(\w+) \?\? 0', '$1 ?? 0m'

Set-Content $filePath $content
Write-Host "Fixed OfferService.cs Id references"
